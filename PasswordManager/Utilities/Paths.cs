namespace PasswordManager.Utilities
{
    class Paths
    {
        public static readonly string fileLocation = "C:\\PasswordManager\\";
        public static readonly string database = "c:\\PasswordManager\\data.db";
        public static readonly string user_data = "c:\\PasswordManager\\user_data.db";
        public static readonly string database_connection = @"URI=file:C:\PasswordManager\data.db";
        public static readonly string user_data_connection = @"URI=file:C:\PasswordManager\user_data.db";        
        public static readonly string hash = "https://raw.githubusercontent.com/Akulav/OfflinePasswordManager/main/PasswordManager/Resources/hash.MD";
    }
}
